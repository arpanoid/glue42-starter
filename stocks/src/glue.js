...
export const setStockDetailsInterop =
    (glue) =>
    ({ RIC, BPOD, Bloomberg, Description, Exchange, Venues, Bid, Ask }) => {
        const isMethodRegistered = glue.interop
            .methods()
            .some(({ name }) => name === 'g42.eas.setStockDetails');
        if (isMethodRegistered) {
            glue.interop.invoke('g42.eas.setStockDetails', {
                RIC,
                BPOD,
                Bloomberg,
                Description,
